...
<body class="sb-nav-fixed">
    ...
    $condition = "WHERE id = $room_id";
    $setCondition = "WHERE schedules.room_id = $room_id";
    if ($room_id === ''){
        $condition = '';
        $setCondition = '';
    }
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    ...
                    ?>
                    <h1 class="mt-4">Schedules <?= "Room Name: " ?> 
                    <?php 
                    if ($room_id === ''){
                        $roomName = 'All Room Schedule';
                    }
                    ?>

                       <?= $roomName; ?><button style="float: right;" type="button" class="btn btn-primary btns" data-bs-toggle="modal" data-bs-target="#addSched">+ Add Schedule</button></h1>
                    <div class="timetable">
                    <?php
                    ...
                    foreach ($timeSlots as $timeSlot) {
                        ...
                        for ($day = 0; $day < 7; $day++) {
                            ...
                            foreach ($schedulesForSlot as $schedule) {
                                echo "<div class='schedule-cell'>" . $schedule['section'] . '<br>' . $schedule['subject'] . '<br>' . $schedule['starttime'] . ' to ' . $schedule['endtime'] ."</div>";
                            }
                        ...
                        for ($day = 0; $day < 7; $day++) {
                            ...
                            foreach ($schedulesForSlot as $schedule) {
                                echo "<div class='schedule-cell'>" . $schedule['name'] . "</div>";
                            }
                    ...
                    <div class="modal-body">
                        <form action="" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <?php
                                $sql = "SELECT room FROM rooms WHERE id = $room_id";
                                
                                $result = mysqli_query($conn, $sql);
                                
                                $row = mysqli_fetch_assoc($result)?>
                                <option value = "<?php echo $room_id; ?>"><?php echo $row['room'];?></option>
                                <?php  ?>
                                </select>
                            </div>
                            <div class="mb-3">
        ...
        if ($conflict) {
            ...
            <script>
                Swal.fire({
                    ...
                    window.location.href = "schedules.php?room_id=<?= $room_id?>";
                });
            </script>
            <?php
        } else {
